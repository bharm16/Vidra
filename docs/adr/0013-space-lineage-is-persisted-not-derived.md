...
> **Amended by [ADR-0022](0022-takes-can-enter-a-session-from-an-upload-the-sketchpad-or-the-studio.md)** — a picture's immediate ancestor is no longer always its words-version: an admitted or refined picture records its origin, every source input, and one display ancestor, and `refine` joins the derived edge kinds.
